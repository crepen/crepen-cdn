...
export class CryptoSymmenticUtil {
    ...
    private async getKey(secret: string): Promise<CryptoKey> {
        ...
        const hash = await this.cryptoFunc.subtle.digest('SHA-256', encoder.encode(secret));
        return this.cryptoFunc.subtle.importKey(
            'raw',
            hash,
            { name: 'AES-GCM' },
            false,
    ...
    encrypt = async (plaintext: string, secret: string = this.symmenticKey) => {
        ...
        const iv = this.cryptoFunc.getRandomValues(new Uint8Array(12));
        const key = await this.getKey(secret);
        const encoded = encoder.encode(plaintext);

        const cipherBuffer = await this.cryptoFunc.subtle.encrypt(
            { name: 'AES-GCM', iv },
            key,
            encoded
        );
    ...
    decrypt = async (ciphertextWithIv: string, secret: string = this.symmenticKey): Promise<string | undefined> => {
        ...
        try {
            ...
            const plainBuffer = await this.cryptoFunc.subtle.decrypt(
                { name: 'AES-GCM', iv },
                key,
                cipherBytes
            );
