import { TokenTypeEnum } from "@crepen-nest/app/api/auth/enum/token-type.auth.request";
...
export class AuthJwtGuard extends AuthGuard('jwt') {
    ...
    handleRequest<TUser = any>(err: any, user: Record<string,object>, info: any, context: ExecutionContext, status?: any): TUser {
        ...
        if (typeof user === 'object') {
            ...
            if (this.whiteListTokenType !== 'all' && this.whiteListTokenType !== user?.payload['type']) {
                ...
                if (typeof (user?.payload['role']) === 'string') {
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
                    const roleList = ((user?.payload['role']) ?? '').split(',')
        ...
        const tokenData: CrepenTokenData = { ...user as object };

        const roleList: UserRoleEnum[] = [];

        const roleKeys = Object.keys(UserRoleEnum) as Array<keyof typeof UserRoleEnum>;

        const roleStrSplit = StringUtil.isEmpty(user?.payload['role'] as string) ? [] : (user?.payload['role'] as string).split('|');
