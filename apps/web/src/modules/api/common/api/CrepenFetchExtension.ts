...
export class CrepenFetchExtenstion {
    static convertCrepenResult = <T = Record<string,unknown> | undefined>(result: FetchApiResult): CrepenApiResult<T> => {
    ...
    static getDefaultErrorResult = <T = Record<string,unknown> | undefined>(e?: Error): CrepenApiResult<T> => {
        return new CrepenApiResult<T>({
            success: false,
