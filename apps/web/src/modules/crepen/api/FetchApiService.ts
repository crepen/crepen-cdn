...
import { CommonApiError } from "../error/api/CommonApiError";
...
export class FetchApi {
    ...
    getResponse = async (type?: FetchApiResultType) => {
        ...
        if (StringUtil.isEmpty(this.baseUrl)) {
            throw CommonApiError.BASE_URL_UNDEFINED;
        }
        else if (StringUtil.isEmpty(this.requestMethod)) {
            throw CommonApiError.REQUEST_METHOD_UNDEFINED;
        }
        ...
        try {
            ...
            throw CommonApiError.FETCH_CONN_FAILED;
        }
    }
}
